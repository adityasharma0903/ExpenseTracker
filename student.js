...
function displayEventDetails(event) {
  ...
  // Registration count
  document.getElementById("registrationCount").innerHTML = `<i class="fas fa-user-check"></i> ${event.registrationCount || "N/A"
    }`

  // Registration deadline
  document.getElementById("registrationDeadline").innerHTML = `<i class="fas fa-hourglass-half"></i> ${event.registrationDeadline || "Opens Soon"
    }`
...
function updatePrizesTab(prizes) {
  ...
  if (prizes.first && prizes.first.amount) {
    ...
    if (firstPrizeElement) {
      ...
      firstPrizeElement.querySelector('.prize-amount').textContent = `₹${Number(prizes.first.amount).toLocaleString()}`;
  ...
  if (prizes.second && prizes.second.amount) {
    ...
    if (secondPrizeElement) {
      ...
      secondPrizeElement.querySelector('.prize-amount').textContent = `₹${Number(prizes.second.amount).toLocaleString()}`;
  ...
  if (prizes.third && prizes.third.amount) {
    ...
    if (thirdPrizeElement) {
      ...
      thirdPrizeElement.querySelector('.prize-amount').textContent = `₹${Number(prizes.third.amount).toLocaleString()}`;
...
function getIconForActivity(title) {
  ...
  if (title_lower.includes('opening') || title_lower.includes('keynote') || title_lower.includes('speaker')) return 'microphone';
  if (title_lower.includes('food') || title_lower.includes('lunch') || title_lower.includes('breakfast') || title_lower.includes('dinner')) return 'utensils';
  if (title_lower.includes('workshop') || title_lower.includes('technical')) return 'chalkboard-teacher';
  if (title_lower.includes('start') || title_lower.includes('kickoff') || title_lower.includes('begin')) return 'rocket';
  if (title_lower.includes('end') || title_lower.includes('submit') || title_lower.includes('deadline')) return 'flag-checkered';
  if (title_lower.includes('award') || title_lower.includes('prize') || title_lower.includes('winner')) return 'trophy';
...
fetch('http://localhost:5000/api/club-events')
  .then(response => {
    if (!response.ok) {
