...
function renderBudgetChart(expenses) {
  ...
  if (total > 0) {
    ...
    Object.entries(categories).forEach(([category, amount], index) => {
      ...
      legendItem.innerHTML = `
        ...
        <span class="legend-text">${category.charAt(0).toUpperCase() + category.slice(1)}: ₹${amount} (${Math.round(percentage * 100)}%)</span>
      `;
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('No budget data available', containerWidth / 2, containerHeight / 2);
  }
}
  ...
  if (total > 0) {
    ...
    ctx.fillStyle = '#64748b';
    ctx.fillText('Total Budget', centerX, centerY + 18);
  } else {
    // No data
...
function renderTimelineChart(events) {
  ...
  months.forEach(month => {
    ...
    if (x >= padding && x <= canvas.width - padding) {
      ...
      ctx.fillText(month.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }), x, canvas.height - padding + 22);
    }
  });
  ...
  visibleEvents.forEach((event, index) => {
    ...
    setTimeout(() => {
      ...
      ctx.beginPath();
      ctx.roundRect(startX, y, width, barHeight, 8);
      ctx.fill();
      
      // Draw event bar
      ctx.fillStyle = '#3b82f6';
      ctx.beginPath();
      ctx.roundRect(startX, y, 0, barHeight, 8);
      ctx.fill();
      ...
      const widthInterval = setInterval(() => {
        ...
        ctx.fillStyle = '#3b82f6';
        ctx.beginPath();
        ctx.roundRect(startX, y, currentWidth, barHeight, 8);
        ctx.fill();
        ...
        if (currentWidth > 50) {
          ...
          ctx.fillText(eventName, startX + 12, y + barHeight / 2);
        }
      }, 20);
      ...
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillText(event.name, startX - 12, y + barHeight / 2);
      
      // Draw date label
      ctx.font = '11px Inter, sans-serif';
      ctx.fillStyle = '#64748b';
      ctx.textAlign = 'center';
      ctx.fillText(formatDate(eventStart), startX, y + barHeight + 18);
    }, 300 + index * 100);
  });
}
...
function renderExpenseCategoryChart(expenses) {
  ...
  if (total > 0) {
    ...
    Object.entries(categories).forEach(([category, amount], index) => {
      ...
      ctx.fillText(category.charAt(0).toUpperCase() + category.slice(1), x + barWidth / 2, canvas.height - padding.bottom + 12);
...
function renderExpenseEventChart(expenses, events) {
  ...
  if (total > 0) {
    const padding = { top: 40, right: 150, bottom: 40, left: 170 };
    const chartWidth = canvas.width - padding.left - padding.right;
    const chartHeight = canvas.height - padding.top - padding.bottom;
    ...
    const barHeight = Math.min(35, chartHeight / Object.keys(eventExpenses).length / 1.5);
    ...
    for (let i = 0; i <= xSteps; i++) {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'top';
      ctx.fillText(`₹${value}`, x, startY + totalBarSpace + barGap + 12);
    }
    ...
    Object.entries(eventExpenses).forEach(([eventId, amount], index) => {
      ...
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillText(eventName, padding.left - 12, y + barHeight / 2);
