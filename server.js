...
// Route to upload a PDF template
app.post('/api/upload-template', uploadPDF.single('template'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ success: false, message: 'No file uploaded' });
    }
    ...
    const template = new Template({
      ...
      path: req.file.path,
      fields: fields,
      uploadedBy: req.body.clubId || 'unknown'
    });
...
// Route to fill a PDF template with data
...
app.get('/api/templates', async (req, res) => {
  try {
    const { clubId } = req.query;
    const query = clubId ? { uploadedBy: clubId } : {};
    
    const templates = await Template.find(query).sort({ uploadedAt: -1 });
